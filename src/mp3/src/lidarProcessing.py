...
class LidarProcessing:
    ...
    def construct_birds_eye_view(self, data):
        ...
        # Geting sensor reading for front_left
        filter_front_left = np.logical_and(x_points > 0, y_points > 0)
        filter_front_left = np.logical_and(filter_front_left, (np.abs(x_points - y_points) < 0.1))
        ...
        # Geting sensor reading for front_right
        filter_front_right = np.logical_and(x_points > 0, y_points < 0)
        filter_front_right = np.logical_and(filter_front_right, (np.abs(x_points + y_points) < 0.1))
        ...
        # Geting sensor reading for rear_right
        filter_rear_right = np.logical_and(x_points < 0, y_points < 0)
        filter_rear_right = np.logical_and(filter_rear_right, (np.abs(x_points - y_points) < 0.1))
