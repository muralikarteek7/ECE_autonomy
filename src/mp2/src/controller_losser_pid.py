...
class vehicleController():
    ...
    def pure_pursuit_lateral_controller(self, curr_x, curr_y, curr_yaw, target_point, future_unreached_waypoints):
        ...
        # x,y = target_point
        x,y = future_unreached_waypoints[1]
        if len(future_unreached_waypoints) > 2:
            x = (x + future_unreached_waypoints[2][0])/2
            y = (y + future_unreached_waypoints[2][1])/2
        dy = y - future_unreached_waypoints[0][0]
        dx = x - future_unreached_waypoints[0][1]
        ...
        if self.prevtime is None:
            ...
            dt = time_curr - self.prevtime


        alpha  = theta - curr_yaw
        error = -alpha
        print(theta, curr_yaw, error, dt)


        controlinput =self.Kp*error + self.Ki*(self.integral + error*dt) + self.Kd*(error - self.prev_error)/dt


        ld = math.sqrt(dy**2+dx**2)
        target_steering = math.atan2(2*self.L*math.sin(controlinput), ld)
        print(target_steering)
        self.integral = self.integral + error*dt
        self.prev_error = error
        self.prevtime = time_curr
